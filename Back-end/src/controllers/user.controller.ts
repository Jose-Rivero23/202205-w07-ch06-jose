...
import { findAll, findOne } from '../models/user.model.js';
...
export class MongooseController<T> {
  ...
  getAllController = async (req: Request, resp: Response) => {
    ...
    resp.send(JSON.stringify(await findAll()));
  };
  ...
  postController = async (req: Request, resp: Response, next: NextFunction) => {
    try {
      ...
      const robot = await Robot.findById(req.body.responsible);
      (robot?.pilots as Array<string>).push(newItem.id);
      await robot?.save();
